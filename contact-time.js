// Contact-local time: infers a contact's timezone from their phone number --
// the NANP area code for US/Canada/Caribbean numbers, the country calling
// code for everything else -- and renders a small "3:42 PM EDT" chip that
// keeps itself current. Plain global script (no module), same convention as
// crm-nav.js / inline-edit.js, so any page can drop it in with one <script>.
//
// It's an estimate, not a fact: an area code says where the number was
// issued, not where the person lives now, and a few area codes straddle a
// zone boundary (the more populous side is used). Countries that span
// several zones (Russia, Brazil, Mexico, Australia, Indonesia) resolve to
// their main zone and are marked approximate ("~") with a tooltip saying so.
(function () {
  if (window.PRAContactTime) return;

  // IANA zone -> space-separated NANP area codes.
  const NANP_BY_ZONE = {
    'America/New_York':
  ...
  // Country calling code -> [IANA zone, approximate?]. Only countries whose
  // main zone is a safe single answer, or flagged approximate when they aren't.
  const CC = {
  ...
  // Phone -> bare international digits ("1" + 10 digits for NANP), or null
  // when the number has no country evidence (a national-format number typed
  // without "+", like a UK "07..." mobile) -- better to show nothing than guess.
  function toInternationalDigits(phone) {
    let s = String(phone || '').trim().replace(/\(0\)/g, ''); // "+44 (0) 7..." trunk zero
  ...
  function resolve(phone) {
    ...
    if (d.startsWith('7')) { // Kazakhstan shares +7 with Russia; its numbers start 76x/77x
  ...
  const fmtCache = {};
  function timeText(tz, approx) {
    try {
      const f = fmtCache[tz] || (fmtCache[tz] = new Intl.DateTimeFormat('en-US', { timeZone: tz, hour: 'numeric', minute: '2-digit', timeZoneName: 'short' }));
      let out = f.format(new Date());
      // en-US only has real abbreviations for US zones (EDT, PST...); everywhere
      // else it prints a bare "GMT+10", which alone doesn't say where -- name
      // the city so it reads "3:36 AM GMT+10 (Sydney)".
      if (/GMT[+-]/.test(out)) out += ` (${tz.split('/').pop().replace(/_/g, ' ')})`;
      return (approx ? '~' : '') + out;
    } catch (e) { return ''; }
  }
  function longZoneName(tz) {
    try {
      return new Intl.DateTimeFormat('en-US', { timeZone: tz, timeZoneName: 'long' }).formatToParts(new Date()).find(p => p.type === 'timeZoneName')?.value || '';
    } catch (e) { return ''; }
  }
  function esc(s) { return String(s).replace(/&/g, '&amp;').replace(/"/g, '&quot;').replace(/</g, '&lt;'); }

  // '' when the zone can't be worked out, so callers can drop it into a
  // ' · '-joined list / label without an empty-chip check.
  function chipHtml(phone, opts) {
    const r = resolve(phone);
    if (!r) return '';
    const text = timeText(r.tz, r.approx);
    if (!text) return '';
    const long = longZoneName(r.tz);
    const title = `Contact's local time${long ? ' — ' + long : ''} · ${r.approx ? 'approximate, ' : ''}estimated from ${r.source}`;
    const cls = 'contact-time' + (opts && opts.className ? ' ' + opts.className : '');
    return `<span class="${cls}" data-tz="${esc(r.tz)}" data-approx="${r.approx ? 1 : 0}" title="${esc(title)}">${esc(text)}</span>`;
  }

  function mount(el, phone, opts) { if (el) el.innerHTML = chipHtml(phone, opts); }

  function refresh() {
    document.querySelectorAll('.contact-time[data-tz]').forEach(el => {
      const t = timeText(el.dataset.tz, el.dataset.approx === '1');
  ...
  window.PRAContactTime = { resolve, chipHtml, mount, refresh };
})();
